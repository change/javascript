...
function convertMemberExpressionToIdentifier(path, j, variableName) {
  findMemberExpressionsForVariable(path, j, variableName).replaceWith((p) => {
    return j.identifier(p.value.property.name);
  });
}
...
function findMethodsAccessedOnVariable(path, j, variableName) {
  ...
  findMemberExpressionsForVariable(path, j, variableName)
    .forEach((p) => {
...
function findRequireForPackage(path, j, packageName) {
  const matches = path
    ...
    .filter(
      (n) => {
        return n.node.arguments.length >= 1 && n.node.arguments[0].value === packageName && n.parentPath.value.id && n.parentPath.value.id.type === 'Identifier';
      }
    );
